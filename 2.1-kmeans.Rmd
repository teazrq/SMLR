...
```{r fig.width=5.5, fig.height=2.7, echo = FALSE}
    par(mfrow=c(1,2))
    par(mar=rep(0.5, 4))
    set.seed(2)
    C = sample(1:2, 6, replace = TRUE)
    plot(x[,1], x[, 2], xaxt = 'n', yaxt = 'n', cex = 2.5, xlim = c(-1.35, 0.45), ylim = c(-1.51, 1.61), pch = 19, col = c("darkorange", "deepskyblue")[C])
...
```{r fig.width=5, fig.height=4, out.width = '55%'}
    library(colorspace)
...
```{r, fig.width=10, fig.height=3, echo = FALSE}
    par(mfrow=c(1,5))
    par(mar=rep(0.2, 4))
    plot(c(0, 400), c(0, 540), xaxt = 'n', yaxt = 'n', bty = 'n', pch = '', ylab = '', xlab = '')
    rasterImage(img, 0, 0, 400, 500)
