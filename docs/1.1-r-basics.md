...
R Markdown is a simple way of creating documents that contain both text (including mathematical equations) and R code chunks. Authors create a `.Rmd` file, which will be fed to [knitr](https://yihui.name/knitr/) to generate the document. Fortunately, RStudio already integrated everything together and all you need is to open up a `.Rmd` file with RStudio and click a button ![](images/knit.png). 
