...
\begin{align}
\underset{\bbeta, \beta_0}{\text{min}} \quad & M \\
...
## Linearly Non-seperable SVM with Slack Variables
...
Besides making the calculation of nonlinear functions easier, using the kernel trick also implies that if we use a proper kernel function, then it defines a space of functions that can be represented in the form of $f(x) = \sum_i \alpha_i K(x, x_i)$, for any $f(x)$ within the space. Hence, as long as we search for a solution within such a space (reproducing kernel Hilbert space, RKHS), then our computational job reduces to solving the $\alpha_i$'s. This is guaranteed by the __Moore–Aronszajn theorem__, which formally states that: Suppose $K$ is a symmetric, positive definite kernel on a set $\cX$. Then there is a unique Hilbert space of functions on $\cX$ for which $K$ is a reproducing kernel. There are numerous articles on the RKHS. Hence, we will not focus on introducing this technique. However, we will later on use some of its properties (the Representor theorem) to motivate the penalized version of SVM. 
...
```{r}
  ...
  legend("topright", c("Zero-one", "Hinge", "Logistic", "Modified Huber"),
         ...
         lwd = 2, cex = 1.5)
```
Again, want to consider nonlinear decision functions. But the above form does not contain a kernel function. The __Representer Theorem__ [@kimeldorf1970correspondence] may help us in this case. The theorem ensures that if we solve the function $f$ with regularization with respect to the norm in the RKHS induced from a kernel function $K$, then the solution must admits a representation of the form
...
where $\bK_{n \times n}$ is the kernel matrix with $\bK_{ij} = K(x_i, x_j)$, and $\bK_i$ is the $i$ the column of $\bK$. This is an unconstrained optimization problem that can be solved using gradient decent if $L$ is differentiable. 