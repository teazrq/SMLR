...
```{r}
    ...
    # use the formula directly to calculate the Cp criterion 
    Cp_sub = RSS_sub + 2*3*summary(lm.fit_sub)$sigma^2
    Cp_sub
```
...
\def\bby{\color{DodgerBlue}{\by^\ast}}
\def\bbe{\color{DodgerBlue}{\be^\ast}}
...
\begin{align}
\E[\color{OrangeRed}{\text{Test Err}}] =& ~\E \lVert \rby - \bX \bbbeta \rVert^2 \\
...
\begin{align}
\E[\color{DodgerBlue}{\text{Train Err}}] =& ~\E \lVert \by - \bby \rVert^2 \\
