...
# (PART) Nonparametric Models through Local Averaging {-}

# K-Neariest Neighber

$K$ nearest neighbor is a simple nonparametric method. Suppose we collect a set of observations $\{x_i, y_i\}_{i=1}^n$, the prediction at a new target point $x_0$ is

$$\widehat y = \frac{1}{k} \sum_{x_i \in N_k(x_0)} y_i,$$
where $N_k(x_0)$ defines the $k$ samples from the training data that are closest to $x_0$. As default, closeness is defined using distance measures, such as the Euclidean distance. Here is a demonstration of the fitted regression function. 
